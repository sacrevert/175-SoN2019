## O.L. Pescott
## started 7 Jan 2019
## Script: 4b_fitGamsForUncertaintyPropagation_DC30_Eng.R
## England

## Run GAMs on multiple single draws from Frescalo date-class results for England
...
tfDC30_Eng <- read.csv(file = "outputs/fres_out_DC1930_Eng/frescalo_190107/Output/Trend.csv", header = T, stringsAsFactors = F)
head(tfDC30_Eng)
tfDC30_Eng <- tfDC30_Eng[order(tfDC30_Eng$Species, tfDC30_Eng$Time),]
rownames(tfDC30_Eng) <- 1:nrow(tfDC30_Eng)

# empty list for species data to go in
spLs <- spLsSim <- vector("list", length(unique(tfDC30_Eng$Species)))
...
spLs <- lapply(unique(tfDC30_Eng$Species), function(x) tfDC30_Eng[tfDC30_Eng$Species == x,])
names(spLs) <- unique(tfDC30_Eng$Species) # add names to list elements
...
spGAMs <- lapply(spLs, FUN = fitGamsSp)
#save(spGAMs, file = "outputs/gams/spGAMs_v2_England_Preds.Rdata")
load(file = "outputs/gams/spGAMs_v2_England_Preds.Rdata")
...
save(spGAMsPredsQs_FIN, file = "outputs/DC1930_England_normGams.Rdata")

pdf("outputs/plots/Rplots_v3_1930Gams_Eng.pdf",
...
pdf("outputs/plots/Rplots_v3_1930Gams_Eng100.pdf",
    width = 4, height = 4, onefile = T)
#x = seq(1970,2018,1)
