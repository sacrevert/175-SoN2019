## O.L. Pescott
## started 7 Jan 2019
## Script: 4b_fitGamsForUncertaintyPropagation_DC30_NI.R
## Northern Ireland

## Run GAMs on multiple single draws from Frescalo date-class results for Northern Ireland
...
tfDC30_NI <- read.csv(file = "outputs/fres_out_DC1930_NI/frescalo_190107/Output/Trend.csv", header = T, stringsAsFactors = F)
head(tfDC30_NI)
tfDC30_NI <- tfDC30_NI[order(tfDC30_NI$Species, tfDC30_NI$Time),]
rownames(tfDC30_NI) <- 1:nrow(tfDC30_NI)

# empty list for species data to go in
spLs <- spLsSim <- vector("list", length(unique(tfDC30_NI$Species)))
#names(spLsSim) <- unique(tfDC$Species)

# create list of species specific Frescalo trend data
spLs <- lapply(unique(tfDC30_NI$Species), function(x) tfDC30_NI[tfDC30_NI$Species == x,])
names(spLs) <- unique(tfDC30_NI$Species) # add names to list elements
...
spGAMs <- lapply(spLs, FUN = fitGamsSp)
#save(spGAMs, file = "outputs/gams/spGAMs_v2_NI_Preds.Rdata")
load(file = "outputs/gams/spGAMs_v2_NI_Preds.Rdata")
...
noData2 <- names(which(unlist(noData))) # 4 or more Tfactor zeros
spGAMsPredsQs_FIN <- spGAMsPredsQs_FIN[!(names(spGAMsPredsQs_FIN) %in% noData2)]
save(spGAMsPredsQs_FIN, file = "outputs/DC1930_NI_normGams.Rdata")

pdf("outputs/plots/Rplots_v3_1930Gams_NI.pdf",
...
pdf("outputs/plots/Rplots_v3_1930Gams_NI100.pdf",
    width = 4, height = 4, onefile = T)
#x = seq(1970,2018,1)
