## O.L. Pescott
## started 7 Jan 2019
## Script: 4b_fitGamsForUncertaintyPropagation_DC30_Brit.R

## Run GAMs on multiple single draws from Frescalo date-class results for Britain
...
tfDC30_Brit <- read.csv(file = "outputs/fres_out_DC1930_Brit/frescalo_190107/Output/Trend.csv", header = T, stringsAsFactors = F)
head(tfDC30_Brit)
tfDC30_Brit <- tfDC30_Brit[order(tfDC30_Brit$Species, tfDC30_Brit$Time),]
rownames(tfDC30_Brit) <- 1:nrow(tfDC30_Brit)

# empty list for species data to go in
spLs <- spLsSim <- vector("list", length(unique(tfDC30_Brit$Species)))
#names(spLsSim) <- unique(tfDC$Species)

#### CHECK SPECIES THAT HAVE ANY ZERO Tfactors ####
spLsCHK <- lapply(spLs, function(x) any(x$TFactor==0))
spLsCHK_TRUE <- which(unlist(spLsCHK))
attr(spLsCHK_TRUE, "names")

# create list of species specific Frescalo trend data
spLs <- lapply(unique(tfDC30_Brit$Species), function(x) tfDC30_Brit[tfDC30_Brit$Species == x,])
names(spLs) <- unique(tfDC30_Brit$Species) # add names to list elements
...
spGAMs <- lapply(spLs, FUN = fitGamsSp)
#save(spGAMs, file = "outputs/gams/spGAMs_v2_Britain_Preds.Rdata")
load(file = "outputs/gams/spGAMs_v2_Britain_Preds.Rdata")
...
save(spGAMsPredsQs_FIN, file = "outputs/DC1930_Britain_normGams.Rdata")

pdf("outputs/plots/Rplots_v3_1930Gams_Brit.pdf",
...
pdf("outputs/plots/Rplots_v3_1930Gams_Brit100.pdf",
    width = 4, height = 4, onefile = T)
#x = seq(1970,2018,1)
