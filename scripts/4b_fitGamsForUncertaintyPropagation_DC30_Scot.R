## O.L. Pescott
## started 7 Jan 2019
## Script: 4b_fitGamsForUncertaintyPropagation_DC30_Scot.R
## Scotland

## Run GAMs on multiple single draws from Frescalo date-class results for Scotland
...
tfDC30_Scot <- read.csv(file = "outputs/fres_out_DC1930_Sco/frescalo_190107/Output/Trend.csv", header = T, stringsAsFactors = F)
head(tfDC30_Scot)
tfDC30_Scot <- tfDC30_Scot[order(tfDC30_Scot$Species, tfDC30_Scot$Time),]
rownames(tfDC30_Scot) <- 1:nrow(tfDC30_Scot)

# empty list for species data to go in
spLs <- spLsSim <- vector("list", length(unique(tfDC30_Scot$Species)))
#names(spLsSim) <- unique(tfDC$Species)

# create list of species specific Frescalo trend data
spLs <- lapply(unique(tfDC30_Scot$Species), function(x) tfDC30_Scot[tfDC30_Scot$Species == x,])
names(spLs) <- unique(tfDC30_Scot$Species) # add names to list elements
...
spGAMs <- lapply(spLs, FUN = fitGamsSp)
save(spGAMs, file = "outputs/gams/spGAMs_v2_Scotland_Preds.Rdata")
load(file = "outputs/gams/spGAMs_v2_Scotland_Preds.Rdata")
...
noData <- lapply(spGAMsPredsQs_FIN, function(x) any(x == "NaN"))
noData2 <- names(which(unlist(noData))) # remove NaNs
spGAMsPredsQs_FIN <- spGAMsPredsQs_FIN[!(names(spGAMsPredsQs_FIN) %in% noData2)]
save(spGAMsPredsQs_FIN, file = "outputs/DC1930_Scotland_normGams.Rdata")

pdf("outputs/plots/Rplots_v3_1930Gams_Scot.pdf",
...
pdf("outputs/plots/Rplots_v3_1930Gams_Scot100.pdf",
    width = 4, height = 4, onefile = T)
#x = seq(1970,2018,1)
